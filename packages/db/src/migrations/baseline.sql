...
-- Fresh-DB baseline: the schema every migration up to v60 (agent_integration)
-- produces. migrateDB applies it to an EMPTY database and stamps v1..v60;
...
CREATE TABLE chat_messages (
      ...
      tool_result TEXT,
      timestamp INTEGER NOT NULL
    , metadata TEXT, chat_id TEXT, agent_ref TEXT, agent_version INTEGER, complete INTEGER NOT NULL DEFAULT 1, turn_role TEXT);
...
CREATE TABLE idempotency_keys (
          ...
          created_at    INTEGER NOT NULL,
          expires_at    INTEGER NOT NULL, request_hash TEXT,
...
CREATE TABLE stage_definitions (
  ...
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
, parent_key TEXT, kind TEXT NOT NULL DEFAULT 'agent');
...
CREATE TABLE stage_runs (
  ...
  item_index INTEGER,
  status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'ready', 'starting', 'running', 'validating', 'awaiting_input', 'waiting', 'retry_wait', 'paused', 'completed', 'failed', 'skipped', 'cancelled')),
  status_reason TEXT,
  version INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE workflow_runs (
  ...
  name TEXT NOT NULL,
  status TEXT NOT NULL DEFAULT 'created' CHECK (status IN ('created', 'starting', 'running', 'waiting', 'paused', 'finalizing', 'cancelling', 'completed', 'failed', 'cancelled')),
  status_reason TEXT,
...
CREATE TABLE scheduler_journal (
  workflow_run_id TEXT NOT NULL REFERENCES workflow_runs(id) ON DELETE CASCADE,
  seq INTEGER NOT NULL,
  message TEXT NOT NULL,
...
CREATE TABLE invocation_uploads (
  id TEXT PRIMARY KEY,
...
CREATE INDEX idx_stage_defs_parent ON stage_definitions(workflow_definition_id, parent_key);

CREATE INDEX idx_chat_workflow_runs_run ON chat_workflow_runs(run_id);
