...
export interface MigrateOptions {
  ...
  /**
   * Force the historic path (legacy bootstrap + every versioned migration
   * from v1) even on an empty database. Only `scripts/generate-db-baseline.ts`
   * and the baseline tests use it; production callers never pass it.
   */
  path?: 'auto' | 'legacy';
}

/** Which route `migrateDB` takes for a given database. */
...
/**
 * Workflow overhaul P00 WP-0.6b (RV-2/RV-3) — the fresh-DB baseline.
 *
 * - An EMPTY database (no user tables, so no `_schema_versions` rows) is
 *   created from `baseline.sql` — the DDL every migration up to
 *   `BASELINE_VERSION` produces, generated by `scripts/generate-db-baseline.ts`
 ...
 *   edited again (R-3).
 */
export function chooseMigrationRoute(sqlite: Database.Database, opts?: MigrateOptions): MigrationRoute {
  if (opts?.path === 'legacy') return { kind: 'legacy' };
  const userTables = sqlite
    .prepare(`SELECT COUNT(*) AS n FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%'`)
    .get() as { n: number };
  if (userTables.n === 0) {
    // A test asking for an intermediate schema below the baseline can only
    // get it from the historic path.
    if (opts?.targetVersion !== undefined && opts.targetVersion < BASELINE_VERSION) return { kind: 'legacy' };
    return { kind: 'baseline' };
  }
  const hasVersions = sqlite
    .prepare(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = '_schema_versions'`)
    .get();
 ...
 *
 * The `sql` of every entry is pinned by `packages/db/src/migrations/migrations.lock.json`
 * (checked by `scripts/check-migrations-lock.mjs` in `pnpm lint`). Every new
 * migration also regenerates the fresh-DB baseline (`pnpm db:baseline`, which
 * moves `BASELINE_VERSION` to the head). Editing
