// `@generatorai/workflow-spec` depends only on zod, so it restates a few
// enums shared defines. This test (here, because shared depends on the spec
// package and not the other way round) keeps them identical.
import { AGENT_MODES, BrowserConfigSchema, HARNESS_PROVIDER_IDS, REASONING_EFFORTS } from '@generatorai/workflow-spec';
