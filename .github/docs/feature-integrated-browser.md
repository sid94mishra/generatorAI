...
Registered via `noServer: true` in [apps/server/src/browser-ws.ts](../../apps/server/src/browser-ws.ts). Coexists with the terminal WS on the same HTTP server. This is the **primary** live-view transport; MJPEG (`/screencast.mjpg`) and the single-frame GET (`/screencast.jpg`) are retained as fallbacks for environments that can't proxy WebSockets cleanly.
