...
export default function ProjectsScreen(): React.ReactElement {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    const found = (agents.data ?? []).filter(
      (a) => !q || a.name.toLowerCase().includes(q) || a.slug.toLowerCase().includes(q),
    );
    const out: Row[] = [];
  ...
  const segments = useMemo(
    () => [
      { value: 'projects' as const, label: 'Projects', count: projects.data?.length ?? 0 },
      { value: 'agents' as const, label: 'Agents', count: agents.data?.length ?? 0 },
    ],
    [projects.data?.length, agents.data?.length],
  );
