...
export function ToolStepRow({ step, nested = false }: { step: ToolStep; nested?: boolean }): React.ReactElement {
  ...
  const tone: RowTone = isAgent && step.status !== 'failed' ? 'info' : toneFor(step.status);
  const color = isAgent && step.status !== 'failed' ? colors.info : statusColor(step.status, colors as Record<string, string>);

  const title = isAgent
    ? `Agent: ${step.agentName ?? 'Sub-agent'}${stepCount > 0 ? ` (${stepCount} ${stepCount === 1 ? 'step' : 'steps'})` : ''}`
    : step.label;
  const subtitle = isAgent ? null : step.shell ? step.shell.command : step.target || null;
      ...
      {isAgent ? (
        <View className="gap-1.5 px-2 py-2">
          {step.children && step.children.length > 0 ? (
            step.children.map((child) => <ToolStepRow key={child.id} step={child} nested />)
          ) : (
...
function GroupRow({ group }: { group: StepGroup }): React.ReactElement {
  ...
  return (
    <RowFrame
      icon={<Icon size={14} color={statusColor(group.status, colors as Record<string, string>)} />}
      title={group.label}
