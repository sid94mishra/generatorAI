...
describe('workspace retention preferences', () => {
  ...
  it('is off by default, so upgrading never starts deleting anything', async () => {
    expect(await readWorkspaceRetentionPreferences(dir)).toEqual({
      enabled: false,
  ...
  it('round-trips what was saved', async () => {
    await writeWorkspaceRetentionPreferences(dir, { enabled: true, retentionDays: 14 });
    expect(await readWorkspaceRetentionPreferences(dir)).toEqual({ enabled: true, retentionDays: 14 });
  });

  it('treats a corrupt state file as OFF rather than throwing', async () => {
    writeFileSync(join(dir, 'workspace-retention.json'), '{ this is not json');
    expect((await readWorkspaceRetentionPreferences(dir)).enabled).toBe(false);
  });

  it('only accepts a literal true for enabled', async () => {
  ...
  describe('clamping the retention period', () => {
    it('keeps values inside the supported range', async () => {
      expect(clampRetentionDays(1)).toBe(1);
      expect(clampRetentionDays(30)).toBe(30);
      expect(clampRetentionDays(365)).toBe(365);
    });

    it('falls back to the DEFAULT rather than the minimum for a bad value', async () => {
    ...
    it('floors a fractional value instead of rejecting it', async () => {
      expect(clampRetentionDays(30.9)).toBe(30);
    });

    it('is applied on the way in as well as on the way out', async () => {
      const saved = await writeWorkspaceRetentionPreferences(dir, {
        enabled: true,
        retentionDays: 99_999,
      });
      expect(saved.retentionDays).toBe(DEFAULT_RETENTION_DAYS);
      expect((await readWorkspaceRetentionPreferences(dir)).retentionDays).toBe(DEFAULT_RETENTION_DAYS);
    });

    it('has a minimum of at least one day', async () => {
      expect(MIN_RETENTION_DAYS).toBeGreaterThanOrEqual(1);
    });
  });

  it('writes the file with restrictive permissions and a timestamp', async () => {
    await writeWorkspaceRetentionPreferences(dir, { enabled: true, retentionDays: 30 });
