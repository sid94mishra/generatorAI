...
// delivery without a key is keyed by its data, so a retried POST replays.
// 202 delivered, 200 replayed, 409 the same key with other data or the wait
// is not waiting any more, 404 an unknown or invalid token, 429 over the
// rate limit (per token and per address).
// ────────────────────────────────────────────────────────────────
...
export function createWorkflowCallbackRoutes(container: Container): Router {
  ...
  router.post('/:token', async (req, res, next) => {
    try {
      ...
      if (!perAddress.allow(req.ip ?? 'unknown') || !perToken.allow(token)) {
      ...
      const idempotencyKey =
        parsed.data.idempotencyKey ?? (header && header.length <= 200 ? header : `callback:${createHash('sha256').update(canonicalJson(data)).digest('hex').slice(0, 32)}`);
