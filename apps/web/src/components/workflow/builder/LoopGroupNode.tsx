// ────────────────────────────────────────────────────────────────
// LoopGroupNode — a loop stage on the builder canvas (P05 §2.1).
//
...
// shows the loop's name, its iteration cap and its exit rules; outer
// stages connect to its handles, body stages only to each other.
// ────────────────────────────────────────────────────────────────

import React, { memo, useCallback } from 'react';
import { Handle, Position, type Node, type NodeProps } from '@xyflow/react';
import { useShallow } from 'zustand/react/shallow';
import { AlertCircle, Bot, Plus, Repeat, SquareTerminal, Trash2, Ungroup } from 'lucide-react';
...
function LoopGroupNodeComponent({ id, data, selected }: NodeProps<Node<StageNodeData>>) {
  ...
  return (
    ...
    >
      {/* Outer edges attach to the loop itself; handles sit at header height. */}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon-sm"
                  aria-label="Add a stage to the loop body"
                  title="Add a stage to the loop body"
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onSelect={() => addInside('agent')}>
                  <Bot className="h-3.5 w-3.5" /> Agent stage
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => addInside('check')}>
                  <SquareTerminal className="h-3.5 w-3.5" /> Check
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => addInside('loop')}>
                  <Repeat className="h-3.5 w-3.5" /> Nested loop
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            <Tooltip content="Unwrap: remove the loop, keep its stages" side="top">
              <Button
                onClick={() => unwrapLoop(id)}
                variant="ghost"
                size="icon-sm"
                aria-label="Unwrap loop"
                className="nodrag h-auto w-auto rounded-lg p-1.5 text-[var(--color-muted-foreground)] hover:bg-[var(--color-accent)] hover:text-[var(--color-foreground)]"
              >
                <Ungroup className="h-3.5 w-3.5" />
              </Button>
            </Tooltip>
            <Tooltip content="Delete the loop and its stages" side="top">
              <Button
