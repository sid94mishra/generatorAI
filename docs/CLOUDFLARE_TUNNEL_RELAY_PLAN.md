...
Separately, the E2EE layer (`packages/relay-protocol/src/e2ee.ts:4-8`) is explicit that it **"applies to BOTH transports: LAN and relay"** — the payload is protected the same way regardless of which pipe carries it. The transcript binds `transport: 'lan' | 'relay' | 'ssh'`, host identity, and both public keys, so a man-in-the-middle (including the tunnel provider itself) can't downgrade or substitute anything without the handshake simply failing to decrypt.

**Conclusion:** we do not need to reconcile or extend the custom relay wire protocol (director/cell assignment, Ed25519 host proofs, the `relay` block in the offer schema) at all. A Cloudflare Tunnel origin can ride in as a plain `reachability: 'public'` entry in the *existing* `endpoints[]` array, get picked up by the *existing* `DirectTransport`, and get protected by the *existing* E2EE handshake. The net-new work is almost entirely about **provisioning the tunnel and injecting its URL into the offer** — not about building a new transport or a new security layer.
...
| Reuse `'relay'` as the E2EE transport tag, or add `'tunnel'`? | **Reuse `'relay'`** | Semantically identical role in the threat model: an untrusted intermediary that must never see plaintext. Adding a new enum value is schema churn for a distinction that doesn't change any security property. Revisit only if UI/diagnostics need to tell them apart later. |
