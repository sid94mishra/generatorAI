...
| 01 | Spec v2, legacy purge, definitions | done | todo | pass (2026-09-25) | 25fd04c..ebb4e82 | Part A (WP-1.1–1.4), part B (WP-1.5) and part C (WP-1.6–1.9) done. Gate: typecheck 52/52; tests = baseline failures only (10: cli 5, core 1, git 2, server 2; the core CRLF and POSIX-path baseline failures are fixed); lint green; no-legacy 78 bans, 0 hits, 11 comments (baseline lowered to 11); migrations-lock 55 locked; db-baseline v55; check:workflow-spec clean; fresh DB + dbcopy-upgrade pass (STATUS). |
